...
class MorenoInputHandler(AbstractInputHandler):
    ...
    def __init__(self:Self, configs: dict, logger:logging.Logger) -> None:
        ...
        self.lightning_model: Optional[L.LightningModel] = None
    ...
    def _load_trained_target_model(self: Self) -> None:
        ...
        if self.configs["moreno"]["representation"] in ["rdkit", "mold2", "transformer_vector", "ECFP_4", "ECFP_6", "MACCS"]:
    ...
    def get_dataloader(self: Self, dataset_indices: np.ndarray, batch_size: int = 64) -> DataLoader:
    ...
    def get_criterion(self:Self) -> torch.nn.modules.loss._Loss:
    ...
    def _dataset_to_list(self: Self, dataset: Dataset) -> Tuple[List[Any], List[Any]]:
